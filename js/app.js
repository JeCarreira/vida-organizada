...
const el = {
  authScreen: document.getElementById('auth-screen'), authForm: document.getElementById('auth-form'), authFeedback: document.getElementById('auth-feedback'), authEmail: document.getElementById('auth-email'), logoutBtn: document.getElementById('logout-btn'), pageTitle: document.getElementById('page-title'), topbarDate: document.getElementById('topbar-date'), pages: [...document.querySelectorAll('.page')], menuItems: [...document.querySelectorAll('.menu-item')], planningTabs: [...document.querySelectorAll('.planning-tab')], planningViews: [...document.querySelectorAll('.planning-view')], sidebar: document.getElementById('sidebar'), mobileMenuBtn: document.getElementById('mobile-menu-btn'), stats: document.getElementById('dashboard-stats'), upcoming: document.getElementById('dashboard-upcoming'), dashboardGreeting: document.getElementById('dashboard-greeting'), dashboardPriorityBoard: document.getElementById('dashboard-priority-board'), prepSummary: document.getElementById('prep-summary'), zoneBFeedback: document.getElementById('zone-b-feedback'), deferFeedback: document.getElementById('defer-feedback'), yearLabel: document.getElementById('year-label'), yearGrid: document.getElementById('year-grid'), calendarLabel: document.getElementById('calendar-label'), calendarGrid: document.getElementById('calendar-grid'), dayEvents: document.getElementById('day-events'), selectedDayLabel: document.getElementById('selected-day-label'), weekLabel: document.getElementById('week-label'), weekGrid: document.getElementById('week-grid'), todayLabel: document.getElementById('today-label'), todayEvents: document.getElementById('today-events'), eventModal: document.getElementById('event-modal'), eventForm: document.getElementById('event-form'), eventFeedback: document.getElementById('event-feedback'), eventSubmitBtn: document.getElementById('event-submit-btn'), quickEventBtn: document.getElementById('quick-event-btn'), selectedDayEventBtn: document.getElementById('selected-day-event-btn'), areaColorDot: document.getElementById('area-color-dot'), areaColorName: document.getElementById('area-color-name'), areaColorNote: document.getElementById('area-color-note'), generateChecklistBtn: null, detailModal: null, detailContent: null, detailTitle: null, detailType: null, detailFeedback: null,
};
...
function renderDashboard() { const now = new Date(); const prepWindowEnd = new Date(now); prepWindowEnd.setDate(now.getDate() + 30); const thisWeekEnd = endOfWeek(now); const weekEvents = state.events.filter((e) => new Date(e.starts_at) >= now && new Date(e.starts_at) <= thisWeekEnd); const prepEvents = state.events.filter((e) => { const days = Number(e.prepare_days_before || 0); if (!days) return false; const eventDate = new Date(e.starts_at); const prepStart = new Date(eventDate); prepStart.setDate(eventDate.getDate() - days); return prepStart <= prepWindowEnd && eventDate >= now && e.status !== 'concluido'; }); const todayEvents = state.events.filter((e) => sameDay(new Date(e.starts_at), now)); const pendingChecklist = state.events.flatMap((event) => (Array.isArray(event.checklist) ? event.checklist : []).map((item) => ({ event, item }))).filter(({ item }) => !item.done); const projectNext = state.events.find((e) => e.area === 'Projeto / Marca' && new Date(e.starts_at) >= now); el.dashboardGreeting.textContent = `Olá — ${now.getHours() < 12 ? 'bom dia' : now.getHours() < 19 ? 'boa tarde' : 'boa noite'}.`; el.stats.innerHTML = `<div class="stat"><div class="muted">O que é mesmo de hoje</div><strong>${todayEvents.length} eventos</strong></div><div class="stat"><div class="muted">Próximo passo</div><strong>${pendingChecklist.length} ações</strong></div><div class="stat"><div class="muted">Preparar sem pressa</div><strong>${prepEvents.length} planos</strong></div><div class="stat"><div class="muted">Esta semana</div><strong>${weekEvents.length} eventos</strong></div>`; el.dashboardPriorityBoard.innerHTML = `<section class="today-column calm-card"><p class="eyebrow">Prioridade real</p><p class="muted">${pendingChecklist[0] ? escapeHtml(pendingChecklist[0].item.text) : 'Hoje faz só o que segura o essencial.'}</p></section><section class="today-column project-card"><p class="eyebrow">Projeto / Marca</p><p class="muted">${projectNext ? `O teu projeto também entra no plano: ${escapeHtml(projectNext.title)}.` : 'O teu projeto não entra no fim da lista.'}</p></section><section class="today-column prep-card"><p class="eyebrow">Já pede preparação</p><p class="muted">${prepEvents[0] ? escapeHtml(prepEvents[0].title) : 'Nada urgente. Mantém só o que vem primeiro.'}</p></section><section class="today-column"><p class="eyebrow">Adiar sem culpa</p><p class="muted handwritten-note">Não é para fazer tudo. É para escolher o essencial e proteger energia.</p></section>`; const upcoming = [...state.events].filter((e) => new Date(e.starts_at) >= now).slice(0, 6); el.upcoming.innerHTML = upcoming.length ? upcoming.map((e) => `<button type="button" class="list-item event-list-item clickable-row" data-event-id="${e.id}" style="--event-color:${eventColor(e.color)}"><div><strong>${escapeHtml(e.title)}</strong><div class="muted">${escapeHtml(eventMeta(e) || e.area || '')}</div></div><span class="muted">${fmtDate(new Date(e.starts_at))}</span></button>`).join('') : '<p class="area-empty">Sem eventos próximos. Podes criar só o próximo passo.</p>'; if (el.prepSummary) { const prepScoped = state.events.filter((e) => ['Natal', 'Aniversário', 'Ocasião especial', 'Viagem', 'Escola / filhos'].includes(e.event_type)); const estimate = prepScoped.reduce((acc, e) => acc + Number(e.budget_estimate || 0), 0); const spent = prepScoped.reduce((acc, e) => acc + Number(e.budget_spent || 0), 0); el.prepSummary.innerHTML = `<div class="stat"><div class="muted">Eventos de preparação</div><strong>${prepScoped.length}</strong></div><div class="stat"><div class="muted">Orçamento previsto</div><strong>${estimate.toFixed(2)} €</strong></div><div class="stat"><div class="muted">Gasto real</div><strong>${spent.toFixed(2)} €</strong></div>`; } }
...
function renderLifeLists() { const byArea = (areas) => state.events.filter((e) => areas.includes(e.area)); const list = (events) => events.length ? events.slice(0, 6).map((e) => `<button type="button" class="list-item clickable-row" data-event-id="${e.id}" style="--event-color:${eventColor(e.color)}"><strong>${escapeHtml(e.title)}</strong><span class="muted">${fmtDate(new Date(e.starts_at))}</span></button>`).join('') : '<p class="area-empty">Ainda sem dados por aqui. Cria só o próximo passo.</p>'; const actions = (events) => events.flatMap((e) => (Array.isArray(e.checklist) ? e.checklist.filter((i) => !i.done).slice(0, 2).map((i) => `<button type="button" class="list-item clickable-row" data-event-id="${e.id}">${escapeHtml(i.text)}<span class="muted">${escapeHtml(e.title)}</span></button>`) : [])).slice(0, 6).join('') || '<p class="area-empty">Sem ações pendentes agora.</p>'; const put = (id, html) => { const node = document.getElementById(id); if (node) node.innerHTML = html; }; put('home-existing', list(byArea(['Casa', 'Rotinas']))); put('home-actions', actions(byArea(['Casa', 'Rotinas']))); put('kids-existing', list(byArea(['Filhos']))); put('kids-actions', actions(byArea(['Filhos']))); put('family-existing', list(byArea(['Família']))); put('family-actions', actions(byArea(['Família']))); put('wellbeing-existing', list(byArea(['Eu', 'Bem-estar']))); put('couple-existing', list(byArea(['Casamento']))); put('project-existing', list(byArea(['Projeto / Marca']))); put('project-actions', actions(byArea(['Projeto / Marca']))); put('papers-existing', list(byArea(['Papéis e burocracia']))); const prepEvents = state.events.filter((e) => ['Natal', 'Aniversário', 'Ocasião especial', 'Viagem', 'Escola / filhos'].includes(e.event_type)); put('prep-existing', list(prepEvents)); put('prep-actions', actions(prepEvents)); put('low-energy-existing', list(byArea(['Rotinas', 'Casa', 'Filhos', 'Bem-estar', 'Projeto / Marca']))); }
function render() { el.topbarDate.textContent = fmtDate(new Date()); renderDashboard(); renderYear(); renderCalendar(); renderWeek(); renderToday(); renderLifeLists(); if (state.detailEventId && !el.detailModal.classList.contains('is-hidden')) renderDetailModal(); }
...
async function boot() { ensureDynamicUi(); el.menuItems.forEach((btn) => btn.addEventListener('click', () => route(btn.dataset.page))); el.planningTabs.forEach((btn) => btn.addEventListener('click', () => setPlanningView(btn.dataset.planningView))); el.mobileMenuBtn.addEventListener('click', () => el.sidebar.classList.toggle('is-open')); el.quickEventBtn.addEventListener('click', () => openEventModal(new Date())); el.selectedDayEventBtn.addEventListener('click', () => openEventModal(state.selectedDate)); document.querySelectorAll('[data-close-modal]').forEach((node) => node.addEventListener('click', closeEventModal)); el.eventForm.elements.area.addEventListener('change', syncAreaColor); el.eventForm.elements.event_type.addEventListener('change', () => { syncSmartDefaults(); syncAreaColor(); }); el.generateChecklistBtn?.addEventListener('click', handleGenerateChecklist);
  document.querySelectorAll('[data-action="open-event"]').forEach((btn) => btn.addEventListener('click', () => openEventModal(new Date())));
  document.querySelectorAll('[data-action="go-planning"]').forEach((btn) => btn.addEventListener('click', () => route('planning')));
  document.querySelectorAll('[data-action="go-low-energy"]').forEach((btn) => btn.addEventListener('click', () => route('low-energy')));
  document.querySelectorAll('[data-action="filter-prep"]').forEach((btn) => btn.addEventListener('click', () => route('prep')));
  document.querySelectorAll('[data-action="quick-fill"]').forEach((btn) => btn.addEventListener('click', () => { openEventModal(new Date()); if (btn.dataset.title) el.eventForm.elements.title.value = btn.dataset.title; if (btn.dataset.area) el.eventForm.elements.area.value = btn.dataset.area; if (btn.dataset.type) el.eventForm.elements.event_type.value = btn.dataset.type; syncSmartDefaults(); syncAreaColor(); }));
  document.querySelectorAll('[data-action="show-zone-b"]').forEach((btn) => btn.addEventListener('click', () => { openEventModal(new Date()); el.eventForm.elements.title.value = 'Férias escolares França · Oise · Académie d’Amiens · Zone B'; el.eventForm.elements.area.value = 'Filhos'; el.eventForm.elements.event_type.value = 'Escola / filhos'; el.eventForm.elements.prepare_days_before.value = '30'; syncSmartDefaults(); syncAreaColor(); if (el.zoneBFeedback) el.zoneBFeedback.textContent = 'Fluxo aberto: cria já o primeiro bloco de datas de França Zone B.'; }));
  document.querySelectorAll('[data-action="defer-rest"]').forEach((btn) => btn.addEventListener('click', () => { if (el.deferFeedback) el.deferFeedback.textContent = 'Feito. Mantém apenas o mínimo de hoje e volta ao resto quando tiveres energia.'; }));
