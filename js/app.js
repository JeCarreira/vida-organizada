...
function route(page) { state.page = page; el.pages.forEach((p) => p.classList.toggle('is-active', p.dataset.page === page)); el.menuItems.forEach((b) => b.classList.toggle('is-active', b.dataset.page === page)); el.pageTitle.textContent = ({ dashboard: 'Entrada', planning: 'Planeamento' })[page] || 'Entrada'; el.sidebar.classList.remove('is-open'); render(); }
