...
async function boot() { ensureDynamicUi(); el.menuItems.forEach((btn) => btn.addEventListener('click', () => route(btn.dataset.page))); el.planningTabs.forEach((btn) => btn.addEventListener('click', () => setPlanningView(btn.dataset.planningView))); el.mobileMenuBtn.addEventListener('click', () => el.sidebar.classList.toggle('is-open')); el.quickEventBtn.addEventListener('click', () => openEventModal(new Date())); el.selectedDayEventBtn.addEventListener('click', () => openEventModal(state.selectedDate)); document.querySelectorAll('[data-close-modal]').forEach((node) => node.addEventListener('click', closeEventModal)); el.eventForm.elements.area.addEventListener('change', syncAreaColor); el.eventForm.elements.event_type.addEventListener('change', () => { syncSmartDefaults(); syncAreaColor(); }); el.generateChecklistBtn?.addEventListener('click', handleGenerateChecklist);
  ...
  document.querySelectorAll('[data-action="quick-fill"]').forEach((btn) => btn.addEventListener('click', () => { openEventModal(new Date()); if (btn.dataset.title) el.eventForm.elements.title.value = btn.dataset.title; if (btn.dataset.area) el.eventForm.elements.area.value = btn.dataset.area; if (btn.dataset.type) el.eventForm.elements.event_type.value = btn.dataset.type; if (btn.dataset.checklist && el.eventForm.elements.checklist_text) el.eventForm.elements.checklist_text.value = btn.dataset.checklist.split('|').map((item) => item.trim()).filter(Boolean).join('\n'); syncSmartDefaults(); syncAreaColor(); }));
  document.querySelectorAll('[data-action="show-zone-b"]').forEach((btn) => btn.addEventListener('click', () => { if (el.zoneBFeedback) el.zoneBFeedback.textContent = 'Fonte ativa: Oise · Académie d’Amiens · Zone B. Podes registar já as pausas escolares como eventos.'; }));
