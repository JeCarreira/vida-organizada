...
function renderDashboard() { const now = new Date(); const prepWindowEnd = new Date(now); prepWindowEnd.setDate(now.getDate() + 30); const thisWeekEnd = endOfWeek(now); const weekEvents = state.events.filter((e) => new Date(e.starts_at) >= now && new Date(e.starts_at) <= thisWeekEnd); const monthEvents = state.events.filter((e) => new Date(e.starts_at).getMonth() === now.getMonth() && new Date(e.starts_at).getFullYear() === now.getFullYear()); const prepEvents = state.events.filter((e) => { const days = Number(e.prepare_days_before || 0); if (!days) return false; const eventDate = new Date(e.starts_at); const prepStart = new Date(eventDate); prepStart.setDate(eventDate.getDate() - days); return prepStart <= prepWindowEnd && eventDate >= now && e.status !== 'concluido'; }); const todayEvents = state.events.filter((e) => sameDay(new Date(e.starts_at), now)); const pendingChecklist = state.events.flatMap((event) => (Array.isArray(event.checklist) ? event.checklist : []).map((item) => ({ event, item }))).filter(({ item }) => !item.done).slice(0, 4); el.dashboardGreeting.textContent = `Olá — ${now.getHours() < 12 ? 'bom dia' : now.getHours() < 19 ? 'boa tarde' : 'boa noite'}.`; el.stats.innerHTML = `<div class="stat"><div class="muted">Prioridades de hoje</div><strong>${todayEvents.length} eventos</strong></div><div class="stat"><div class="muted">Esta semana</div><strong>${weekEvents.length} eventos</strong></div><div class="stat"><div class="muted">Preparar</div><strong>${prepEvents.length} planos</strong></div><div class="stat"><div class="muted">Ações pendentes</div><strong>${pendingChecklist.length}</strong></div>`; el.dashboardPriorityBoard.innerHTML = `<section class="today-column"><p class="eyebrow">Hoje só preciso de…</p><p class="muted">${todayEvents[0] ? escapeHtml(todayEvents[0].title) : 'uma ação pequena e possível.'}</p></section><section class="today-column"><p class="eyebrow">Se estiveres sem energia</p><p class="muted">Escolhe só 3 mínimos e deixa o resto para depois, sem culpa.</p></section>`; const upcoming = [...state.events].filter((e) => new Date(e.starts_at) >= now).slice(0, 6); el.upcoming.innerHTML = upcoming.length ? upcoming.map((e) => `<button type="button" class="list-item event-list-item clickable-row" data-event-id="${e.id}" style="--event-color:${eventColor(e.color)}"><div><strong>${escapeHtml(e.title)}</strong><div class="muted">${escapeHtml(eventMeta(e) || e.area || '')}</div></div><span class="muted">${fmtDate(new Date(e.starts_at))}</span></button>`).join('') : '<p class="muted">Sem eventos importantes próximos.</p>'; if (el.prepSummary) { const prepScoped = state.events.filter((e) => ['Natal', 'Aniversário', 'Ocasião especial', 'Viagem', 'Escola / filhos'].includes(e.event_type)); const estimate = prepScoped.reduce((acc, e) => acc + Number(e.budget_estimate || 0), 0); const spent = prepScoped.reduce((acc, e) => acc + Number(e.budget_spent || 0), 0); el.prepSummary.innerHTML = `<div class="stat"><div class="muted">Eventos de preparação</div><strong>${prepScoped.length}</strong></div><div class="stat"><div class="muted">Orçamento previsto</div><strong>${estimate.toFixed(2)} €</strong></div><div class="stat"><div class="muted">Gasto real</div><strong>${spent.toFixed(2)} €</strong></div>`; } }
