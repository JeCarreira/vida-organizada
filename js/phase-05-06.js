  ...
  return `
    <section class="phase-panel is-hidden" data-panel="edit">
      ...
      <form id="phase-edit-form" class="form">
        ...
        <div class="form-row">
          <label>Início<input name="starts_at" type="datetime-local" value="${toDateTimeLocal(event.starts_at)}" required></label>
          <label>Fim<input name="ends_at" type="datetime-local" value="${toDateTimeLocal(event.ends_at)}"></label>
        </div>
        <div class="form-row">
          <label>Tipo<select name="event_type">${optionList(typeOptions, event.event_type)}</select></label>
          <label>Área<select name="area">${optionList(areaOptions, event.area)}</select></label>
        </div>
        <div class="form-row">
          <label>Estado<select name="status">${optionList(statusOptions, event.status)}</select></label>
          <label>Repetição<select name="recurrence">${optionList(recurrenceOptions, event.recurrence)}</select></label>
        </div>
        <label>Preparar com antecedência<select name="prepare_days_before">${optionList([['0','No próprio dia'],['1','1 dia antes'],['3','3 dias antes'],['7','1 semana antes'],['14','2 semanas antes'],['30','1 mês antes'],['60','2 meses antes'],['90','3 meses antes']], String(event.prepare_days_before || 0))}</select></label>
...
function renderChecklistEditor(event) {
  ...
  return `
    <section class="phase-panel is-hidden" data-panel="checklist-edit">
      <h4>Editar checklist</h4>
      <p class="muted">Uma ação por linha. Ao guardar, as ações que já estavam concluídas mantêm o estado sempre que o texto for igual.</p>
      <form id="phase-checklist-form" class="form">
        <textarea name="checklist" rows="12">${escapeHtml(text)}</textarea>
        <button class="primary-btn" type="submit">Guardar checklist</button>
      </form>
    </section>`;
}
...
function renderBudgetPanel(event) {
  ...
  return `
    <section class="phase-panel is-hidden" data-panel="budget">
      <h4>Compras e orçamento</h4>
      <form id="phase-budget-form" class="form">
        <div class="budget-grid">
          <label>Orçamento previsto (€)<input name="budget_estimate" type="number" min="0" step="0.01" value="${Number(event.budget_estimate || 0)}"></label>
          <label>Já gasto (€)<input name="budget_spent" type="number" min="0" step="0.01" value="${Number(event.budget_spent || 0)}"></label>
        </div>
        <label>Lista de compras<textarea name="shopping_list" rows="8" placeholder="Uma compra por linha: item | quantidade | loja | valor previsto | comprado">${escapeHtml(shoppingText)}</textarea></label>
        <button class="primary-btn" type="submit">Guardar compras e orçamento</button>
      </form>
    </section>`;
}
  ...
  content.insertAdjacentHTML('beforeend', `
    <section class="phase-enhancement">
      <div class="phase-actions">
        <button type="button" class="text-btn" data-open-panel="edit">Editar evento</button>
        <button type="button" class="text-btn" data-open-panel="checklist-edit">Editar checklist</button>
        <button type="button" class="text-btn" data-open-panel="budget">Compras e orçamento</button>
      </div>
      <section class="grouped-checklist-card">
        <p class="eyebrow">Checklist organizada</p>
        ${renderGroupedChecklist(event)}
      </section>
...
async function saveEdit(form) {
  ...
  await updateEvent(activeEventId, patch);
}
...
async function saveBudget(form) {
  ...
  await updateEvent(activeEventId, {
    budget_estimate: Number(fd.get('budget_estimate') || 0),
    budget_spent: Number(fd.get('budget_spent') || 0),
    budget_currency: 'EUR',
    shopping_list: parseShopping(fd.get('shopping_list')),
  });
}
...
function installHandlers() {
  document.addEventListener('click', (event) => {
    ...
    if (opener?.dataset.eventId) {
      ...
      window.setTimeout(refreshEnhancement, 120);
    }
    ...
    if (panelButton) {
      ...
      root?.querySelector(`[data-panel="${panelButton.dataset.openPanel}"]`)?.classList.remove('is-hidden');
    }
  }, true);
  ...
  document.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (submit) submit.textContent = oldText || 'Guardar';
    } finally {
      window.setTimeout(() => { if (submit) { submit.disabled = false; submit.textContent = oldText || 'Guardar'; } }, 700);
      busy = false;
    }
  }, true);

  new MutationObserver(refreshEnhancement).observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });
}
...
function installStyles() {
  ...
  document.head.insertAdjacentHTML('beforeend', `<style id="phase-05-06-styles">
    .phase-enhancement{border-top:1px solid var(--line);margin-top:18px;padding-top:16px}.phase-actions{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.grouped-checklist-card,.phase-panel{border:1px solid var(--line);background:rgba(255,255,255,.58);padding:14px;margin:12px 0}.dossier-check-group{margin:14px 0}.dossier-check-group h5{margin:0 0 8px;text-transform:uppercase;letter-spacing:.06em;color:var(--soft-ink);font-size:.78rem}.phase-panel.is-hidden{display:none}.budget-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}@media(max-width:900px){.budget-grid{grid-template-columns:1fr}}
  </style>`);
}
